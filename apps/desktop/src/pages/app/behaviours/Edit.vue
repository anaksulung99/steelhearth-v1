...
import type { UpdateBehaviourDto } from "@/api";
...
const form = reactive<UpdateBehaviourDto>({
  name: "",
  type: "DEFAULT_READER",
  minDwellSeconds: 30,
  maxDwellSeconds: 120,
...
onMounted(async () => {
  const b = await behaviourStore.fetchOne(id);
  if (b) {
    form.name = b.name;
    form.type = b.type;
    form.minDwellSeconds = b.minDwellSeconds;
    form.maxDwellSeconds = b.maxDwellSeconds;
    form.minScrollCount = b.minScrollCount;
    form.maxScrollCount = b.maxScrollCount;
    form.scrollSpeedMin = b.scrollSpeedMin;
    form.scrollSpeedMax = b.scrollSpeedMax;
    form.enableInternalNav = b.enableInternalNav;
    form.maxInternalClicks = b.maxInternalClicks;
    form.customClickSelectors = (b.customClickSelectors ?? []).map((s) => ({
      selector: s.selector,
...
async function submit() {
  ...
  const payload = { ...form } as UpdateBehaviourDto;
  if (payload.type !== "CUSTOM_CLICKER") {
    payload.customClickSelectors = undefined;
  }
  const result = await behaviourStore.update(id, payload);
  submitting.value = false;
  if (result) router.push(`/app/behaviours/${id}/detail`);
}
</script>
...
<template>
  <div class="p-6 w-full space-y-6">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold">Edit Behaviour Profile</h1>
        <p class="text-sm text-muted-foreground">
          Update session behaviour configuration
        </p>
      </div>
    </div>
