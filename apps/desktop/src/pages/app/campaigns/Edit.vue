...
import type { CampaignClickSelector, FingerprintProfile, UpdateCampaignDto } from "@/api";
...
function addClickSelector() {
  ...
  form.clickSelectors = [
    ...current,
    { selector: "", selectorType: "css", description: "", order: current.length },
  ];
...
function normalizeSelectorType(selectorType?: string) {
  return selectorType === "xpath" || selectorType === "elementId" ? selectorType : "css";
}
  ...
  <div class="p-6 w-full space-y-6">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-muted-foreground">Update campaign configuration</p>
      </div>
    </div>
    ...
    <form v-else class="space-y-4" @submit.prevent="submit">
      <div class="rounded-lg border bg-card p-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1 col-span-2">
            <label class="text-sm font-medium">Campaign Name <span class="text-destructive">*</span></label>
            <input
              v-model="form.name"
              type="text"
              required
          ...
          <div class="space-y-1 col-span-2">
            <label class="text-sm font-medium">Target URL <span class="text-destructive">*</span></label>
            <input
      ...
      <div class="rounded-lg border bg-card p-4 space-y-4">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="space-y-1">
            ...
            <input v-model.number="form.totalSessionsTarget" type="number" min="0" class="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div class="space-y-1">
            <label class="text-sm font-medium">Daily Limit</label>
            <input v-model.number="form.dailyLimit" type="number" min="1" class="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div class="space-y-1">
            <label class="text-sm font-medium">Sessions/Hour</label>
            <input v-model.number="form.sessionsPerHour" type="number" min="1" class="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
        </div>
      </div>
      ...
      <div class="rounded-lg border bg-card p-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1">
            ...
            <select v-model="form.browserEngine" class="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
          ...
          <div class="space-y-1">
            ...
            <select v-model="form.behaviourProfileId" class="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
              <option :value="null">None (use default)</option>
              <option v-for="b in behaviourStore.items" :key="b.id" :value="b.id">
                {{ b.name }}
              </option>
            </select>
          </div>
          <div class="space-y-1 col-span-2">
            ...
            <select v-model="form.fingerprintProfileId" class="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
              <option :value="null">None (use default)</option>
          ...
          <div class="flex items-center gap-2 col-span-2">
            <input id="headless" v-model="form.headless" type="checkbox" class="rounded" />
            <label for="headless" class="text-sm font-medium">Headless mode</label>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-lg border bg-card p-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <label class="flex items-center gap-2 rounded-md border px-4 py-3 cursor-pointer" :class="form.geoMode === 'proxy' ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'">
            <input v-model="form.geoMode" type="radio" value="proxy" class="accent-primary" />
            <span class="text-sm font-medium">Proxy Pool</span>
          </label>
          <label class="flex items-center gap-2 rounded-md border px-4 py-3 cursor-pointer" :class="form.geoMode === 'manual' ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'">
            <input v-model="form.geoMode" type="radio" value="manual" class="accent-primary" />
        ...
        <div v-if="form.geoMode === 'proxy'" class="space-y-2">
          ...
          <div v-if="proxyStore.groups.length === 0" class="rounded-md border border-dashed p-4 text-center text-sm text-muted-foreground">
          ...
          <div v-else class="space-y-2">
            <label
              ...
              :class="(form.proxyGroupIds ?? []).includes(g.id) ? 'border-primary bg-primary/5' : ''"
            >
              <input
                type="checkbox"
              ...
              <span class="text-xs text-muted-foreground">({{ g._count?.proxies ?? 0 }})</span>
            </label>
          </div>
        </div>
        ...
        <div v-else class="space-y-1">
          ...
          <input
            ...
            class="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
          />
          <p class="text-xs text-muted-foreground">Comma-separated ISO 3166-1 alpha-2 country codes.</p>
        </div>
      </div>
      ...
      <div class="rounded-lg border bg-card">
        ...
        >
          ...
          <Icon :icon="showClickSelectors ? 'material-symbols:expand-less' : 'material-symbols:expand-more'" class="size-4" />
        </button>
        <div v-if="showClickSelectors" class="px-4 pb-4 space-y-4 border-t">
          <div class="flex justify-end pt-3">
            <button type="button" class="flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-xs font-medium hover:bg-muted" @click="addClickSelector">
              <Icon icon="material-symbols:add" class="size-3.5" />
              Add Selector
            </button>
          </div>
          <div v-if="!form.clickSelectors?.length" class="rounded-md border border-dashed p-5 text-center text-sm text-muted-foreground">
            No selectors
          </div>
          <div v-else class="space-y-3">
            <div v-for="(sel, idx) in form.clickSelectors" :key="idx" class="rounded-md border bg-muted/30 p-3 space-y-2">
              <div class="flex items-center justify-between">
                <span class="text-xs font-medium text-muted-foreground">#{{ idx + 1 }}</span>
                <button type="button" class="text-destructive hover:opacity-70" @click="removeClickSelector(idx)">
              ...
              <div class="grid grid-cols-3 gap-2">
                <div class="col-span-2 space-y-1">
                  ...
                  <input v-model="sel.selector" type="text" placeholder=".btn-cta, //button, or submit-button" class="w-full rounded-md border bg-background px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
                <div class="space-y-1">
                  <label class="text-xs font-medium">Type</label>
                  <select v-model="sel.selectorType" class="w-full rounded-md border bg-background px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
              ...
              <div class="space-y-1">
                ...
                <input v-model="sel.description" type="text" placeholder="Optional" class="w-full rounded-md border bg-background px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex justify-end gap-3">
        <button type="button" class="rounded-md border px-4 py-2 text-sm hover:bg-muted" @click="router.back()">
          Cancel
        </button>
